    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ __('Rename this layup without changing its layers.') }}
            </p>
        </div>
    </x-slot>
            ...
            <x-panel class="p-6">
                <form method="POST" action="{{ route('suppliers.layups.update', [$supplier, $layup]) }}" class="space-y-6">
                    ...
                    <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                        <x-link-button variant="secondary" href="{{ route('suppliers.layups.show', [$supplier, $layup]) }}">
                            {{ __('Cancel') }}
                        </x-link-button>
                        <x-primary-button>{{ __('Save Layup') }}</x-primary-button>
                    </div>
                </form>
            </x-panel>
        </div>
